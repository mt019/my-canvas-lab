...
// Vite dev server 對路徑段裡的 %-編碼 sub-delim（如逗號 %2C、& %26）會失配、掉進 SPA fallback
// （iframe 變空白）。這些字元在 URL 路徑段中本就合法（RFC 3986 pchar），正式站靜態伺服器也服務得了，
// 故 encodeURIComponent 後把被過度編碼的路徑合法 sub-delims 還原成原始字元。
const _PATH_SAFE = { '%2C': ',', '%26': '&', '%3D': '=', '%2B': '+', '%24': '$', '%3B': ';', '%40': '@', '%3A': ':' };
const encSeg = (s) => encodeURIComponent(s).replace(/%2C|%26|%3D|%2B|%24|%3B|%40|%3A/g, (m) => _PATH_SAFE[m]);
const pdfUrl = (rel) => `${PDF_BASE}/${rel.split('/').map(encSeg).join('/')}`;
...
function FileRow({ f, onView }) {
  ...
  const ext = f.外部連結 && !f.相對路徑;  // 有本倉 PDF 就頁內看，沒下載到才退官網
  const open = () => (ext
...
export default function JirsForeignLaw() {
    ...
    for (const r of reports) {
      for (const f of r.檔案) {
        ...
        out.push({
          ...
          相對路徑: f.相對路徑, 外部連結: f.外部連結, 頁數: f.頁數, 位元組: f.位元組,
        });
      }
    }
    return out;
  ...
  // pool 只取本倉有 PDF（相對路徑）者，換一篇的迴圈才不會彈到官網新分頁而斷掉。
  const rollPool = useMemo(() => cases.filter((c) => c.相對路徑), [cases]);
  const [rollCase, setRollCase] = useState(null);
  const openCase = useCallback((c) => {
    setRollCase(c);
    setViewing({ 相對路徑: c.相對路徑, 顯示名: c.案名, 頁數: c.頁數, 位元組: c.位元組 });
  }, []);
          ...
          <div className="lg:grid lg:grid-cols-[14rem_minmax(0,1fr)] lg:gap-10">
            ...
            <aside className="contents lg:block lg:py-5 lg:sticky lg:top-0 lg:h-screen lg:self-start lg:overflow-y-auto lg:border-r lg:pr-5">
              <div className="flex items-center justify-between gap-2 pt-5 lg:flex-col lg:items-start lg:gap-3 lg:pt-0">
                {/* 返回鍵走共用元件＋全站配置，不寫死落點；外觀與隱形都在 BackLink 裡。 */}
                <BackLink />
                ...
                <section className="space-y-4">
                  ...
                  <ul>
                    {filteredCases.slice(0, 500).map((c, i) => (
                      <li key={`${c.相對路徑}-${i}`} className="flex flex-wrap items-baseline gap-x-3 gap-y-0.5 border-b border-line-soft py-2 last:border-b-0">
                        <button type="button"
                          onClick={() => (c.相對路徑
                            ? setViewing({ 相對路徑: c.相對路徑, 顯示名: c.案名, 頁數: c.頁數, 位元組: c.位元組 })
                          ...
                          title={c.相對路徑 ? c.案名 : `${c.案名}（官網開啟）`}>
                          {c.案名}{c.相對路徑 ? null : <ExternalLink size={11} className="ml-1 inline align-baseline text-ink-faint" />}
                        </button>
