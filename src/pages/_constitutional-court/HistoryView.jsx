...
const PROV_SEGMENTS = [ // token-exempt: 沿革機關時間軸分類色（資料，非樣式）
  ...
  // 色位取自 ERA_TONE：釋字色位與案件圖「解釋」同 rose、憲判色位與「判決」同 blue，
  // 兩圖對得起來；1949 前三段（red/slate/amber）案件圖不含。由 validate:colors 鎖住色帶。
  { key: '大理院', 機關: '大理院・統字', 起: '1913-01-15', 迄: '1927-10-22', 號數: '統字 1–2012', color: catPair(ERA_TONE.大理院), card: '大理院', 定性: '統一解釋法令，約法解釋不在其權限，惟屢援約法補法令之缺' },
  { key: '最高法院', 機關: '最高法院・解字', 起: '1927-12-25', 迄: '1928-11-20', 號數: '解字 1–245', color: catPair(ERA_TONE.最高法院), card: '最高法院', 定性: '不滿一年的過渡期，體例承襲大理院，統一解釋權旋移司法院' },
  { key: '司法院', 機關: '司法院・院字／院解字', 起: '1929-02-16', 迄: '1948-06-23', 號數: '院字 1–2875；院解字 2876–4097', color: catPair(ERA_TONE.司法院), card: '司法院', 定性: '訓政時期最高司法機關統一解釋；行憲後空窗期直接解釋新憲法' },
  { key: '釋字', 機關: '大法官・釋字', 起: '1949-01-06', 迄: '2021-12-24', 號數: '釋字 1–813', color: catPair(ERA_TONE.釋字), card: '大法官', 定性: '依憲法作成、具憲法位階的憲法解釋與統一解釋' },
  { key: '憲判', 機關: '憲法法庭・憲判', 起: '2022-01-04', 迄: null, 號數: `憲判 ${data.統計.判決}＋實體裁定 ${data.統計.實體裁定}（迄今）`, color: catPair(ERA_TONE.憲判), card: '大法官', 定性: '大法官改組為憲法法庭，以判決運作，納入裁判憲法審查' },
];

const PROV_STAGES = [
  {
    key: '大理院', 機關: '大理院解釋（統字）', 期間: '1913–1927', 號數: '統字 1–2012 號',
  ...
  {
    key: '最高法院', 機關: '最高法院解釋（解字）', 期間: '1927–1928', 號數: '解字 1–245 號',
  ...
  {
    key: '司法院', 機關: '司法院解釋（院字／院解字）', 期間: '1929–1948', 號數: '合計 4,097 件（院字 993 未公布）',
  ...
  {
    key: '大法官', 機關: '大法官解釋（釋字）與憲法法庭裁判（憲判）', 期間: '1948–迄今', 號數: `釋字 1–813 號 · 憲判 ${data.統計.判決} 件（迄今）`,
    text: '1946 年政協憲草首次寫入「大法官」：「司法院即為國家最高法院……由大法官若干人組織之」，設計仿美國聯邦最高法院。1947 年施行的《中華民國憲法》第 78 條明定司法院解釋憲法並統一解釋法律命令，第 79 條設大法官。第一屆大法官 1948 年就任，1949 年 1 月 6 日公布釋字第 1 號；至 2021 年 12 月 24 日釋字第 813 號止，共 813 號。個別意見書制度亦在此期間逐步成形：1949–1958 年間僅依 1948 年《司法院大法官會議規則》運作，並無公布個別意見書的機制，釋字第 1–79 號均無大法官個別意見書；1958 年《司法院大法官會議法》首度開放不同意見書（首見同年釋字第 80 號），1993 年更名之《司法院大法官審理案件法》明定解釋併同公布各大法官之協同、不同意見書（首見釋字第 315 號），至 2022 年憲法法庭更逐案公開各庭員的逐項立場表。其間憲法多次修正調整大法官制度與職權：1992 年增訂政黨違憲解散案件之審理；1997 年第四次修憲改定大法官任期為八年、不分屆次個別計算、不得連任，並自 2003 年起施行，為使任期交錯，該年提名的十五位大法官中八位（含正副院長）任期四年、其餘八年，此後每四年更新約半數，任命時點自此與大法官組成的更替直接相關；2005 年增訂正副總統彈劾案件。2019 年公布《憲法訴訟法》，2022 年 1 月 4 日施行，大法官改組成憲法法庭，以裁判（憲判字）形式運作，並納入裁判憲法審查。',
  },
];
...
const PROV_LINKS = [
  ...
  { label: '維基百科・不同意見書（1958 年《大法官會議法》開放、釋字 80 號首見）', href: 'https://zh.wikipedia.org/wiki/不同意見書' },
];
...
const PROV_MARKERS = [ // 制憲/憲訴法＝政體級（帶色）；意見書/交錯任期＝大法官期制度里程碑（--cc-eyebrow 次級）
  { yr: provYear('1946-11-15'), label: '制憲', color: 'var(--cc-accent)' },
  { yr: 1958, label: '意見書', color: 'var(--cc-eyebrow)' },
  { yr: 2003, label: '交錯任期', color: 'var(--cc-eyebrow)' },
  { yr: 2022, label: '憲訴法', color: 'var(--cc-type-judgment)' },
];

// 沿革敘事卡 → 案件索引深連：階段 key 對映到「機關」facet 值（大法官段含大法官＋憲法法庭＝行憲後）。
const STAGE_機關 = { 大理院: '大理院', 最高法院: '最高法院', 司法院: '司法院', 大法官: '行憲後' };
