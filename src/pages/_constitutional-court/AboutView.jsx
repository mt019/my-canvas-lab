    ...
    <div className="max-w-3xl">
      <section className="border-t border-[var(--cc-line)] py-5">
        ...
        <div className="mt-2 space-y-2 text-[13.5px] leading-relaxed text-[var(--cc-ink-mid)]">
          ...
          <p>
            主題分類、稅法子主題與審查結論先由規則初步標註。行憲後案件（大法官釋字＋憲法法庭憲判）的處分結果另經逐件核定，卡片以處分模式標示（如「純解釋」「違憲宣告」，詳見下節）；規則歸不進六格結論的行憲前統一解釋標「其他」。意見書作者與共同具名關係解析自官方檔名；早期意見書僅收於抄本合訂檔者，已從抄本檔名拆出（卡片上仍連向抄本 PDF）。中期釋字（約第 100–400 號）的意見書常整卷收於抄本且檔名未列作者，該時期的意見書統計為下限而非全貌。
          </p>
          <p>
            「審查基準」欄依湯德宗三級架構（寬鬆／中度／嚴格）機標：本院自釋字第 578 號起才明確區分寬嚴審查基準，之前案件不標；機標只認理由書明示字樣，多數案件為「未明示」，同案命中多級者標「多重基準」。參與大法官名單：釋字取自官方頁解釋文／理由書末尾的大法官署名列（813 件全覆蓋，署名列不含迴避或未參與評議者），憲法法庭判決與裁定取自官方合議庭名單欄。
          </p>
        </div>
      </section>
      <section className="border-t border-[var(--cc-line)] py-5">
        <p className="text-[11px] font-bold uppercase tracking-[0.08em] text-[var(--cc-eyebrow)]">審查結論類型學</p>
        <h2 className="text-base sm:text-lg font-bold text-[var(--cc-title-ink)]">六個軸，把每件的處分結構拆細</h2>
        <div className="mt-2 space-y-2 text-[13.5px] leading-relaxed text-[var(--cc-ink-mid)]">
          <p>
            規則初步標的「審查結論」只有六格（合憲／違憲／違憲定期失效／法令解釋／補充前解釋／變更前解釋），看不出處分的細部結構。類型學在其上另立六個分析軸，替行憲後每一件拆出更細的處分樣態：
          </p>
          ...
          <p>
            涵蓋全部 {typedTotal} 件行憲後案件（大法官釋字＋憲法法庭憲判），逐件核定並經雙盲對照。拆開後可見，規則初步歸為「其他」的件逾半其實是純解釋與統一解釋。
          </p>
          <p>
            「案件索引」的類型學 6 軸篩選涵蓋全部 {typedTotal} 件行憲後案件；「案件時間軸」的「主題×處分模式」矩陣即以這批案件的 A 軸主處分呈現各領域分佈，並把實體審查結果與其他處分分兩段排開。
          </p>
        </div>
      </section>
      <section className="border-t border-[var(--cc-line)] py-5">
