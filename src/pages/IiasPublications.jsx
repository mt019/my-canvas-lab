...
const NOTABLE_AUTHORS = [
  ...
  'Jerome A. Cohen', 'Jacques deLisle', 'William P. Alford',
];
const NOTABLE_LABEL = {
...
const MAIN_TABS = [
  ...
  { id: 'china', label: '中國法專題', Icon: Globe },
...
const SERIES = [
  ...
  },
  {
    name: '司法制度實證研究',
    re: /司法制度實證研究/,
    note: '2008 與 2011 兩冊，法實證研究那條線的論文集。',
  },
  {
    name: '法的理性—吳庚教授紀念論文集',
    re: /法的理性/,
...
const INSTITUTE_TIMELINE = [
  { year: '2004', text: '設籌備處，湯德宗任主任', links: [{ label: '湯德宗', tab: 'curator' }] },
  {
    year: '2005',
...
// 中國法／兩岸主題判別：兩岸四地法律發展系列與香港特刊整本在題內，其餘刊物取逐篇命中者。
...
// 跨出版品的篇章表（篇名／作者／出處／頁碼／取得）。篇章檢索、中國法專題、所史三處共用，
// 免得同一張表在檔內抄三份、改一處漏兩處。
...
export default function IiasPublications() {
  ...
  const [chinaOpen, setChinaOpen] = useState(() => new Set()); // 專題頁展開的旗艦卷冊 id（預設全收，點封面展開）
  ...
  // 中國法／兩岸主題：旗艦整本（兩岸四地系列＋香港特刊）＋散見他刊的逐篇命中者。
    ...
    <main className={styles.workspace}>
      <aside className={styles.sidebar}>
                ...
                <div>
                  ...
                  <dd>
                    <ul className={styles.factItems}>
                      ...
                      <li>法研所首任所長，2011 年成所後接任、同年 9 月底卸任</li>
                      <li>司法院大法官，2011–2019</li>
                    </ul>
                  </dd>
                </div>
        ...
        {/* 創所所長・前大法官——側欄入口只擺在總覽與篇章檢索這兩個側欄本來就空的分頁。
            完整清單、期刊架、系列、作者、中國法專題、所史各有自己該放的東西（分類篩選、
          ...
          <>
            ...
            <section className={`${styles.panel} ${styles.aboutPanel}`}>
              ...
              <div className={styles.aboutBody}>
                ...
                <p>
                  怎麼逛：<b>完整清單</b>依出版品逐本展開目次；<b>期刊架</b>只收《中研院法學期刊》、點封面看該期目次；<b>篇章檢索</b>跨全集搜書名、篇名、作者；<b>作者</b>看發表最密的研究群、點姓名讀其著作；<b>中國法專題</b>把兩岸四地與港澳法制的卷冊與篇章聚在一起；<b>所史</b>收這批書裡談這個所自己的篇章（序、致詞、十週年文集）。總覽側欄的<b>隨機翻閱</b>會抽一篇實質論文；開啟後可沿著同作者、同一本書「順藤摸瓜」讀下去。
                </p>
                <p className={styles.aboutReflectHead}>整理者的話</p>
                <p>
                  法律學研究所在中央研究院裡是很年輕的一個所。2004 年設籌備處，湯老師接下籌備處主任，把它從一紙計畫、幾個人，一手籌了七年。2011 年終於掛牌成所，他成為第一任所長。就在同一年，他獲提名為大法官，臨危受命，於是他就從法律所退休，赴任大法官去了。今年是法律所十五年，以研究機構的年紀論，它還在長身體，而這批出版品，差不多就是它從無到有的那些年，一頁一頁累起來的東西。
                </p>
                <p>
                  翻完這幾百篇，浮上來的是一個埋頭做慢事的地方。最長的一條線是《憲法解釋之理論與實務》，本站收到的從 2005 年第四輯到 2025 年第十二輯，前三輯早於法律所。它是研討會的論文集：每一輯先請一位外國憲法學者做主題演說，Mark Tushnet、Michel Troper、Aharon Barak、Stefan Korioth、David Law、長谷部恭男都來過，台灣的學者接著各談各的題目，有比較憲法的方法論，也有針對個別釋字的評析。湯德宗的「階層式比例原則」就是 2009 年第六輯下冊那篇〈違憲審查基準體系建構初探〉提出來的。這條線做兩件事，把外面的討論接進來，同時讓大法官的解釋在同代學者面前受檢，二十年沒有斷過。
                </p>
                <p>
                  另一條線是法實證研究，設所時就寫進了規劃。湯德宗在司改十週年論壇的致詞裡說得明白，「司法制度的比較研究與司法行為的實證研究」是設所規劃書選定的六大重點研究領域之一，籌備處 2004 年成立起就在開拓，隨即辦起「司法制度實證研究國際研討會」；後來這條線主要由張永健接手做大，另設了法實證研究資料中心。方向我認為是對的，做出來的東西則要說老實話，多半還很陽春。典型的一篇長這樣。到法源用條號撈一批地方法院判決，分層隨機抽樣、人工編碼，再跑敘述統計、卡方檢定和邏輯斯迴歸，報告某種分割方式占幾成、某個變數的係數顯不顯著。〈分割共有物判決之實證研究〉從 2,702 筆抽出可用的 500 件；〈越界建築訴訟之實證研究〉撈到 1,181 筆，編碼後剩 192 筆，這 192 筆不夠跑迴歸，只好另撈一批 156 筆來補；有些交叉表分到只剩十幾二十筆觀察值，照樣下了判斷。要測民法修法有沒有改變法院行為，靠的是修法前後對照，而撐住這個對照的假設（修法前後的案件組成沒變）寫在註腳裡，沒有對照組。
                </p>
                <p>
                  更麻煩的是材料本身。判決書只留得住走到判決的案件，和解、撤回、根本沒進法院的那些，一開始就不在資料裡；判決書寫的是法官願意寫下來的理由，他實際上被什麼推著走，紙面上看不到。越界建築那一批，明確談到越界者心理狀態的判決只有 8 筆，可是故意或重大過失正是民法第 796 條的關鍵要件。這樣的材料能穩當回答的問題，比作者想問的窄得多。經濟分析那一半跟數判決那一半也常常分開走，〈越界建築之經濟分析〉2013 年、〈越界建築訴訟之實證研究〉2014 年，一篇推理論、一篇報數字，理論預測了什麼、數據驗到了什麼，接得並不緊。這些工作把現象清點出來了，離檢驗假設還有一段距離。
                </p>
                <p>
                  這條線真正留得住的東西在別處：資料中心那樣的基建，還有它拉來的國際合作。也在於這個所容得下打自己的人。2020 年第 27 期，賀劍評張永健《物權法之經濟分析》的六十二頁書評，和張永健五十二頁的答辯，前後排在一起刊出。肯把顛覆自己旗艦著作的文章登在自己的期刊上，比任何一篇實證論文都更能說明它的體質。
                </p>
                <p>
                  這個所的視野是兩岸四地和更大的世界，而那條線是這批書裡最好看、也最讓人難受的一條。陳弘毅記得清楚：湯德宗費了很大力氣，創建一年一度的兩岸四地法學研討會，輪流在台灣、大陸、香港、澳門開；法研所另設大陸與港澳訪問學人計畫，每年給名額，對多位大陸學者來說，在中研院訪問的那幾個月是他們學術生涯的轉捩點。本站收到輪在台灣主辦的三本論文集，2006、2010、2014。
                </p>
                <p>
                  翻 2006 那本會愣一下。上冊整卷在比四地的違憲審查制度：北京大學的張千帆寫〈從憲法到憲政——中國大陸憲法審查制度的歷史、現狀與未來〉，社科院的莫紀宏寫大陸違憲審查的發展趨勢，而替他們寫評論文的是湯德宗，題目叫〈試論大陸確立違憲審查制度的途徑〉。一位台灣學者在那個場合認真替對岸盤算憲法審查該怎麼建起來，對岸的學者就坐在同一張桌子上聽。每篇論文都配一到三篇來自其他三地的評論，這個格式本身就是那幾年的空氣：大家默認彼此在同一條路上，只是走到不同的地方。
                </p>
                <p>
                  2010 那屆談法學教育與法律人怎麼養成，2014 那屆談華文法學與研究方法，出書已經拖到 2018 年。這批出版品裡，這個系列停在那一屆。再出現香港的專號，已經是 2022 年的《香港法治之變局》，篇名一路是恐懼之術、自由憲政秩序的崩壞、一國兩制的難產與早夭、總體國家安全觀對法治人權的侵蝕；2006 年並排坐著的那些大陸高校學者，這本裡一個也沒有。李建良在出刊辭裡寫，香港的學術自由同樣受到國安法影響，因此「國際社會間可以研討香港議題的平臺日益顯得重要」，希望批判性的觀點「可以持續找到思辨空間」。從替對岸設想違憲審查該怎麼建，到替香港留一塊還能把話說完的地方，中間隔了十六年。
                </p>
                <p>
                  往外的那一半倒是還在。外文出版品那一類是同一個方向的產物，Empirical Legal Analysis、Law and Economics of Possession、Selection and Decision in Judicial Process around the World，把各國學者請進同一本書裡寫；孔傑榮、戴傑、安守廉這些讀中國法的人，從十週年文集到香港特刊都還在這批書裡留字。開闊的是這一面，收窄的是另一面，兩件事同時發生在同一個所裡。
                </p>
                <p>
                  到今天（{data.meta.資料截至.slice(0, 4)} 年），它有的是這樣一批東西：四類出版品、將近八百篇文章、一群還在發表的研究者，和一份跟這座島的公法、行政、憲政纏在一起的紀錄。它以後會長成什麼樣，誰也說不準；法治從來不是一個研究所守得住的。可是從這些文字看得出來，有一群人，願意慢慢地、認真地，替它把每一步記下來。
                </p>
        ...
        {tab === 'china' && (
          <>
            <section className={styles.panel}>
              ...
              <p className={styles.authorIntro}>
                本所自 2006 年起以「兩岸四地法律發展」系列持續蒐羅台灣、中國大陸、香港、澳門四地的法制比較，2022 年另有香港法治變局特刊。這裡把這批以中國法與兩岸議題為主軸的卷冊集中起來，並收攏散見於各期期刊、十週年文集裡談兩岸與港澳法制的篇章。這是依主題整理的選讀，不是完整書目。點卷冊展開目次。
              </p>
              {chinaTopic.flagship.map((p) => (
                <PubCard
                  key={p.id}
                  pub={p}
                  open={chinaOpen.has(p.id)}
                  onToggle={() => setChinaOpen((prev) => {
                    const next = new Set(prev);
          ...
          <>
            ...
            {instituteRows.anniversary && (
              <section className={styles.panel} id="iias-anniv">
                ...
                <p className={styles.authorIntro}>
                  成所十年時編的《研之得法》，是這個所最集中的一份自述。前半是所內外的人回頭看這十年，王汎森、翁岳生、王澤鑑、許宗力、陳弘毅、Jerome A. Cohen、William P. Alford、Cass R. Sunstein 等人各寫一篇；後半是所內研究者交代自己這些年在做什麼。要了解這個所怎麼長起來的，從這本翻起最快。
                </p>
                <PubCard
                  pub={instituteRows.anniversary}
                  open={!chinaOpen.has(`anniv-${instituteRows.anniversary.id}`)}
                  onToggle={() => setChinaOpen((prev) => {
                    const next = new Set(prev);
