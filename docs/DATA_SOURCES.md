...
| `1142/phenom-court-data` | 獨立站 `phenom-court` 的 SHA-pinned export（`.court-snapshot/` → `prepare-data.mjs`）；Canvas 無本地投影 | 2026-08-02：Canvas 端的頁面、私有元件與四份資料快照全部刪除（`src/pages/_constitutional-court/`、`ConstitutionalCourt.jsx`、`constitutionalCourt{,-reasoning-fulltext,-pre1947-fulltext}.json`、`daliyuanCollation.json`、`typologyReport.md`、`public/data/constitutionalCourt-opinions/`），只留 `/all` 外鏈與舊路徑 308 轉址；`validate:cccutover` 改為斷言這些路徑不存在。字號檢索、快照取用、浮層三支閘已搬進 `phenom-court` 的 `verify:policy` | **已全量覆核**：初版239件中信度釋字已逐案審核；現為憲法解釋651／統一解釋162，813件釋字均高信度。資料倉庫有機器可讀覆核登錄、專用 validator 與維護 SOP；未來新邊界案回到人工覆核佇列。既有6軸 `結論類型` 與本欄正交，不互相覆寫 |
| `1142/phenom-notes-data` | 獨立 `phenom-notes` 的 clean `export:web` snapshot；Canvas 無本地投影 | 2026-08-01：data `e1ea1bb` 經 `phenom-ops` run `30696570015` 發布至 Cloudflare Pages；Canvas commit `235e539` 移除所有 Notes 建置與同步，只保留公開 `/notes/*` 代理及 `/all` 外鏈 | 低——production 固定完整 web/data SHA、逐檔 SHA-256、preview smoke 後 promotion 同一 artifact；草稿產物不可進 production |
