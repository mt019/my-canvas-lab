...
### 行高只有三個值（2026-08-13 訂，機器守）

正文 `leading-[1.85]`、次要文字與清單 `leading-[1.8]`、程式碼與表格 `leading-[1.7]`。
共用的 `Prose` 元件本來就是這三個值，手刻頁面照抄，不要憑感覺填。

來歷：德川頁的對讀分頁寫 `leading-[2]`、導言寫 `1.95`、年表寫 `1.8`，站主翻分頁時看得出
「這一頁不一樣」，卻指不出哪裡不一樣——每頁只差一點點。全站掃過去，1.75、1.9、1.95、2.05
各有幾處，散在五個檔。這種偏移不會壞掉任何東西，所以沒有人會回報，只會一直長。

`scripts/validate-prose-typography.mjs` 掛在 `verify:policy` 上擋；要新增一個值，先改本節
與該腳本的 `SCALE`，讓它成為全站的選項。只此一處的例外寫進該腳本的 `EXCEPTIONS` 並附理由
（目前一條：術語表詞欄 `2.2`，要留得下標在字頭上的假名 ruby）。
...
## 排版壞了，先改共用層（2026-08-12 使用者裁定，全局底層）

一個頁面的版面壞掉，第一個要問的是這個畫面由哪個元件畫出來。畫它的是 `src/components/lab/` 的共用元件或 `@phenomcanvas/ui` 的共用殼時，改那裡，不在頁面裡另寫一份樣式蓋掉它——同一個元件在別的文章、別的站上長的是同一個樣子，只修眼前這一頁等於把同一個毛病留在其餘每一頁。這條不必等人交代。

同日修的兩處，都在共用層：

1. **選項清單的對齊。** `Quiz.jsx` 的選項原本是 `flex-wrap` 的膠囊，按鈕預設置中，長選項折到第二行就成了置中的多行文字（違反「會折行的文字一律靠左」），寬度還隨字數參差。現在的規則：任一選項超過 14 個字，該題整組選項各占一整列、`w-full`、`text-left`；全部都短（是／否這種）才留在同一列並置中。判準寫在元件裡，不寫在頁面裡。
2. **資料層字串裡的 LaTeX。** `.mdx` 的 `$…$` 由 remark-math 在建置時轉換，JSX 用 `<Math tex="…" />`，而**從資料倉 JSON 讀出來的字串兩者都不經過**——洗牌那篇的選項因此把 `$2^k$` 原樣印在頁面上。共用元件 `src/components/lab/MathText.jsx` 處理這種字串：字串就切開渲染，不是字串就原樣傳過去，所以元件可以無條件包上它。已接的欄位有 quiz 的 prompt／options／explain、statements 的四個欄位、圖表 caption、引用浮卡與註釋的 locator。`npm run validate:math` 第二條規則守著它：資料裡出現含 `$…$` 的新欄位，而欄名不在腳本的 `MATH_RENDERED_FIELDS` 裡，建置就失敗。

## 分步圖解與動畫的數字（2026-08-13 使用者明令「全網學學設計、動畫設計、學習留痕」）

規則的完整版連同出處在 `~/.claude/skills/步驟動畫設計.md`，本節只留全站要守的數字。
已套用的實作是洗牌篇的 `SplitExample.jsx`。

- **分步控制列先分型**（2026-08-13 使用者裁定）：有順序的**敘事型圖解**，主控制是
  上一步／下一步兩顆按鈕，步驟名降成進度標示（「3/5・交錯」，`aria-live="polite"`），
  不當按鈕；**wizard 表單**（要回頭改答案）才用可點步驟列，當前步實心反白加粗、
  `aria-current="step"`，狀態不靠顏色單獨區分。兩型通用：名字用該步的動作、觸標
  `py-2` 起跳、序號與名字之間放真的空格不放 margin。
- **時長**：微互動 120–180ms、元素進出 200–350ms、分步圖解的一步 400–600ms；功能性 UI
  不超過 500ms。進場比出場長三到五成。
- **easing**：空間移動永不 linear。預設 MD3 標準曲線 `cubic-bezier(0.2, 0, 0, 1)`；
  進場 `(0.05, 0.7, 0.1, 1)`、出場 `(0.3, 0, 1, 1)`。
- **波次（stagger）**：一群元素按目的位置逐個延遲，總延遲 500ms 以內；元素多就壓小單位
  延遲（52 根柱 × 7ms），不是砍元素。方向跟閱讀方向或語意走。
- **無障礙**：`motion-reduce:transition-none` 一律要接；inline style 只放 duration／delay／
  timing，transition-property 留在 class，media query 才蓋得掉。動畫只動 transform 與
  opacity。
- **連續性**：分步圖解的元素 key 綁身分（牌值）不綁位置，尺寸不隨步驟變，動畫只有移動，
  讀者才看得出「它還是它」。
