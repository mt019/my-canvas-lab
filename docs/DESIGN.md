...
Notion 的 tag／badge 色系從不出醜，原因是三條紀律，本站色彩系統一律照做：

1. **色彩永遠是「淡底＋深墨色文字」一對，不是實色色塊。** 底色（bg）近白、只帶極淡色傾向；真正看得見色相的是文字／邊框的墨色（ink），且墨色本身彩度也不推到頂。單一實色填滿的按鈕、tag、整條 bar，一律禁止（呼應下方「金箔限文字筆畫」與「正文區底色只准 paper」兩條，這是同一條鐵律的第三個變體）。
2. **顏色永遠小面積、當鉻件用，不當版面主體。** 一個 badge、一個 tag、一條邊框——不是整片背景、不是整顆按鈕、不是整條圖表 bar。
3. **一個畫面最多一處撞色（pop）。** 三種以上互相對比的色相並排（例如三色橫排的圖例、三色一組的色塊），即使每個顏色單獨看都不醜，湊在一起也會讀成「跳色」——這正是 `palettes.js` 裡 `pop` 角色「一個畫面最多出現一處」的既有規則，不是新規則，只是這次補上「不只 pop 本身要克制，任何跨色系並排都要克制」的推論。
