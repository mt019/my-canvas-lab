...
### `Brief` ＋ `brief/*`（個人簡報站，2026-07-17 新建；**區塊仍寫死在頁面裡，見下**）

站的形狀：`/brief` 儀表板（`pages/Brief.jsx`）＋ `/brief/events` 活動曆（`pages/brief/Events.jsx`，
`PAGE_META` 標 `listed: false`——門口列它，首頁列門口）＋ `pages/brief/data.js`（**資料契約在前端
的唯一入口，兩頁都從它讀**）。資料倉 `../brief-data`，`npm run sync` 投影一個檔
`src/data/brief-events.json`（14 MB → 512 KB）。契約見資料倉 `docs/data-contract.md`，**動手前讀它**。

**`/brief` 是儀表板不是連結列表**：一份「活動曆 →」的索引，讀者還是得點進去才知道有沒有事。
六區由上而下——這 7 天關門的／接下來的活動／正在進行中／讀的東西／售票節目（預設收起）／
資料來自哪裡＋盲區。**排序只在區之間做一次**（照錯過的代價），各區內部一律照時間，不按「重要」
重排——替他決定什麼重要正是這個站要殺的病。

`/brief/events` 一頁三種看法（條列／月曆／交叉表），共用一組篩選，全部進網址（`useTabParams`：
`mode`／`sources`／`hosts`／`rows`／`cols`）。**來源是逐個可切的**，不是「我關注的／全部」兩顆
按鈕——後者的「全部」會把 444 檔售票節目倒進同一條清單，把 70 場講座沖掉。

**這兩頁自己算的只有「剩幾天」**；其餘每個字都來自資料倉。
...
**固定檢查**：`npm run check:brief`（需另開 `npm run dev`）。14 項，一律「從 JSON 自己算一次再跟
畫面上的字比」，不比對寫死的期望值。**它存在的理由**：480/444 那個錯，build 四閘全綠、eslint 全綠、
零 console error——每一道都過，而畫面在說謊。負向測試做過：把 bug 放回去，它從 14/14 掉到 13/14 並
指名該項。

**這個站被使用者退回四次，教訓全在 `docs/DESIGN.md`**：「資料頁的形狀」「資料驅動的頁不准被單一
來源綁架」「**也不准被單一種類綁架**」三節。**動這個站之前先讀那三節。**

**下一輪要做的（2026-07-17 使用者裁示）**：`/brief` 現在六區裡五區是活動，因為資料端是 528 場活動
對 65 篇讀的東西。使用者原話：「活動只是整個 brief 裡面很小的一部分」——他要的還有知識、新聞、
研究報告、市場分析。**做法**：(1) 資料倉先補四類來源（研究報告與論文＝NBER／CEPR／NEP／Crossref
繞 IMF-OECD；政策講辭＝Fed／ECB／SNB／WTO／EC Taxation／Tax Foundation；學者與科技一手＝Krugman／
MR／Milanovic／Cochrane／Wren-Lewis Substack、OpenAI／DeepMind／Claude Code releases；社群趨勢＝
Reddit `.rss`），全部已在計畫檔附錄實測可用；(2) **前端的區塊改成從 `sources[]` 的 `collection`＋
`kind` 長出來**，加一個來源＝多一區，不再寫死六區。**證券市場分析是時間序列不是清單**（同一個數字
每天更新），要走 `components/lab/chart/` 做圖表，使用者已裁示這一輪先排隊。

**還沒有的**：標記層（「我要去」／「留著」）。使用者第五輪就點名要，它要寫進 private 的標記檔，
沒有假裝有。OPENTIX 即時票況走前端現抓（倉裡刻意不存剩餘票數，幾小時就過期），與標記層一起做。
