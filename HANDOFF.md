...
### `Brief` ＋ `brief/*`（個人簡報站，2026-07-17 新建，第九輪改成區塊由資料生成）
...
**分頁切 collection，不切 kind**（`useTabParams`，`?view=reading|events|sources`，預設 `reading`）：
切 kind 會生出 12 個分頁、擠到橫滾，而且資料倉每加一種 kind 分頁列就長一格。**「這 7 天關門的」
釘在分頁之上，不屬於任何分頁**——錯過就沒了的東西不能藏在別的分頁後面。預設落在「讀的東西」是
對「這是什麼站」的回答，不是對「哪批筆數最多」的回答（使用者：「活動只是整個 brief 裡面很小的
一部分」）。

**每一區只出每個來源的前 4 件，深的那一層在內頁。** `/brief/reading` 三欄：左欄來源逐個可切
（按 kind 分組，kind 標題整類一起切）＋關鍵字；中欄 247 篇連摘要；右欄自動目次。`/brief/events`
一頁三種看法（條列／月曆／交叉表），共用一組篩選，全部進網址。**Reading 沒有月曆與交叉表**：
那兩種看法回答「時間怎麼排」，而論文沒有行程可排——一份資料多種看法的前提是每種看法各自回答
得了一個真問題，不是每頁都湊三個分頁。
...
**固定檢查**：`npm run check:brief`（需另開 `npm run dev`）。24 項，一律「從 JSON 自己算一次再跟
畫面上的字比」，不比對寫死的期望值。**它存在的理由**：480/444 那個錯，build 閘門全綠、eslint 全綠、
零 console error——每一道都過，而畫面在說謊。

第九輪新增的兩類與它們的負向測試（都跑過、都咬到、都指名）：
...
**下一輪**：(1) **標記層**（「我要去」／「留著」）第五輪點名至今未做，要寫進 private 標記檔；
(2) **OPENTIX 即時票況走前端現抓**（倉裡刻意不存剩餘票數），與標記層一起做；(3) **證券市場分析是
時間序列不是清單**（同一個數字每天更新），要走 `components/lab/chart/` 做圖表，使用者已裁示排隊。
...
- 2026-07-17 **字號精準檢索**（frontend-only，**DONE**，零資料 repo 改動）。原本搜尋框對
  ...
  **固定檢查**：`npm run validate:cases`（`scripts/validate-case-query.mjs`，已掛進 `npm run build`
  ...
  讓裸數字當字號，兩次都當場失敗並指名項目；還原回綠。**這支檢查第一次跑就咬出 `113-1` 的
