...
// `/constitutionalcourt/:tab`、`/zhujiahua/:zhuTab`）是拿 tab id 去 SEO 表裡查 slug，
...
const SITES = [
  {
    label: '憲法法庭案例庫',
    page: 'src/pages/ConstitutionalCourt.jsx',
    seo: 'src/pages/_constitutional-court/seo.js',
    seoVar: 'CC_TABS_SEO',
    // 分頁列從 `const tabs = [{ id: 'timeline', … }]` 長出來，'index' 留在站首頁、
    // 照設計沒有自己的網址。
    linkedSlugs: (src) => {
      const block = src.match(/const tabs\s*=\s*\[([\s\S]*?)\n\];/);
      if (!block) return null;
      return [...block[1].matchAll(/\bid:\s*'([^']+)'/g)].map((m) => m[1]).filter((id) => id !== 'index');
    },
  },
  {
    label: '朱家驊研究室',
