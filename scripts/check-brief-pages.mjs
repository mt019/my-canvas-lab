 ...
 * 零 console error，**每一道都過，而畫面在說謊**。能偵測到它的只有一件事：真的把頁面打開，
 * 拿畫面上的數字去對資料。
 *
...
try {
  ...
  /* 論文沒有「來不及」——讀的東西那幾區不准出現倒數。
     **只掃那幾區**：報裡面讀的東西與活動在同一頁上，而活動的倒數是對的。舊版靠「分頁標籤
     以下」切範圍，那在切 collection 的版面成立，換成報就沒意義了——它查到的是隔壁區。
     倒數一定帶數字（inDays() 印的是「21 天後」）：不能只找「天後」，OECD 的來源說明裡資料倉
     自己寫著「出版日有時排在幾天後」，那是散文，對它報錯只會訓練人忽略這道檢查。 */
  ...
  /* ---- 標記層：留著／我要去
   *
   * 這幾項要查的是同一件事：**「留著」是不是真的留得住。** 資料倉每個來源只送最新 12 篇
