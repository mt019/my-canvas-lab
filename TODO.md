...
- [ ] **索引卡片無限捲動＋預取，取代「顯示更多」按鈕**（前端，使用者 2026-07-11 提；之後做）。現況 `IndexView` `limit` state 初值 30（L667）、`sorted.slice(0, limit)`（L771）、底部「顯示更多」鈕 `setLimit(limit+50)`（L899–905）。使用者要：(1) 初始預覽多一點（提高初值）；(2) 視窗下拉到接近底部時**自動加載**（IntersectionObserver 掛一個 sentinel，進入視口即 `setLimit`）；(3) **提前預取**、加載量放大，消除卡頓感，不要手動點。保留鍵盤/無 JS 後備可再議。純前端。
...
- [ ] 類型學 follow-up ③（**2026-07-11 裁定執行、進行中**）：348 件非違憲行憲後件補標 6 軸（清單＝資料 repo `data/materials/貼標-剩餘清單.json`，已排除人工佇列 23 件；~6 輪 opus×2＋gate `--agree`，第 1 輪 60 件進行中）。新資訊在 C/D/E/F 軸。完成後補跑 `app-json→validate→sync→build`。另兩件仍待裁決：23 件人工佇列真分歧（opus-deep 或人工，勿同批重跑）；行憲前 5040 是否延伸（見 plan 檔方案，建議僅 C 軸規則式或延後）。分類器信度/效度/穩健度評估：資料 repo `engineering/LOG.md` 2026-07-11 條。
