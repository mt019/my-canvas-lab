...
## 統計站術語層（2026-07-13 上線，後續）
- [ ] 術語頁的 SEO/AEO：每頁 `DefinedTerm` JSON-LD（答案引擎最愛定義頁），與全站 hreflang／預渲染那輪一起做
- [ ] 第二篇文章上線時，把它的新術語補進 `data/glossary/`（骨架已定，加資料夾即可；閘門會擋斷鏈）
- [ ] 目前一個術語只在文章裡標第一次出現處；若日後覺得標得太少，規則要一起改，別逐篇即興
